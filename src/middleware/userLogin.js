...
const fUserLogin = async (req, res, next) => {
  ...
  if (req.headers.token) {
    if (req.headers.token === 'k3mb4nggul4') {
      ...
      if (currentExpired > tokenExpired) {
        logError('filter-controller.fUserLogin', { message: 'Webtoken is expired' });
        // next(createError(401, 'Webtoken is expired'), { expose: true }); // token expired
        res.status(401).json({
